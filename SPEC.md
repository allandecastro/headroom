...
Update **detection** is GitHub-API based. `UpdateChecker::check()` queries `GET https://api.github.com/repos/allandecastro/headroom/releases/latest` (public, no token; a User-Agent is required) and compares the `tag_name` to the compile-time `CARGO_PKG_VERSION` via a `major.minor.patch` semver compare (`is_newer`, tolerant of a `v` prefix and pre-release suffixes). Returns `UpdateInfo { version, url }` when the remote is strictly newer, else `None`; drafts/prereleases are ignored. Surfaced to the renderer through the `get_update` / `check_for_update_now` IPC commands and the `update-available` event.

Update **install** ("Update now") is handled by the `install_update` command (`commands.rs`) on top of `tauri-plugin-updater`: it downloads the signed artifact described by the release's `latest.json`, installs it, emits `update-progress` for the UI, and `app.restart()`s into the new version. Artifacts are signed with a minisign key (`tauri signer generate`); the public key lives in `tauri.conf.json` (`plugins.updater.pubkey`), the private key + password are CI secrets (`TAURI_SIGNING_PRIVATE_KEY[_PASSWORD]`), and `bundle.createUpdaterArtifacts` is on. The release workflow's `updater-manifest` job merges the matrix's per-platform `latest.json` files into one manifest (Windows + Linux AppImage entries). Self-install is gated to platforms that support it — **Windows (MSI)** and **Linux AppImage**; **Linux `.deb`** and **direct-DMG macOS** fall back to opening the release page, the old manual flow. **macOS via Homebrew** is special-cased: rather than the in-app updater (which needs notarized signing, still deferred), `install_update` detects a brew-managed install (`brew list --cask headroom`) and launches `brew upgrade --cask headroom` in a **detached Terminal** — it must run outside the app process because brew quits Headroom mid-upgrade to swap the bundle. Auto-install works only *from* the first release that ships the plugin onward; the Homebrew path tracks the auto-published `allandecastro/homebrew-headroom` tap (see the release workflow's `update-tap` job).
